...
imageListEl.addEventListener(`click`, onProductClick);

function onProductClick(event) {
  event.preventDefault();
  const imageCardEl = event.target.closest(`.gallery-item`);
  if (!imageCardEl) return;
  const original = imageCardEl.querySelector(".gallery-image").dataset.source;
  const currentImage = images.find((image) => {
    return image.original === original;
  });

  const instance =
    basicLightbox.create(`<img src="${currentImage.original}" alt="${currentImage.description}">
  `);
  instance.show();
}
